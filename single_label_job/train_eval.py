...
def evaluate(config, model, data_iter, test=False):
    ...
    acc = metrics.f1_score(labels_all,predict_all,average=None)
    acc =acc[0]
    if test:
...
def predict(config,model,test_iter):
    ...
    with torch.no_grad():
        for texts in test_iter:
            outputs = model(texts)
            pred_i = torch.max(outputs.data, 1)[1].cpu().numpy()
            if predict_alls == []:
                predict_alls = pred_i
                continue
            predict_alls = np.concatenate((predict_alls, pred_i), axis=0)
            num += len(texts)
            if num % 1000 == 0:
                print('已经预测{}条数据'.format(num))
